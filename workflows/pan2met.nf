...
include { PPANGGOLIN_EXTRACT_SEQUENCES } from "../modules/local/ppanggolin/fasta.nf"
...
workflow PAN2MET {
    ...
    if (params.genomes) {
        ...
        // 
        pangenome = PPANGGOLIN_WORKFLOW(params.genomes).out.pangenome
        ch_versions = ch_versions.mix{ PPANGGOLIN_WORKFLOW.out.versions }
        proteins = PPANGGOLIN_EXTRACT_SEQUENCES(pangenome).out.pangenome_family_proteins
        ch_versions = ch_versions.mix{ PPANGGOLIN_EXTRACT_SEQUENCES.out.versions }
        ...
        proteins = Channel.fromPath(params.proteome)
    } else {
        error "No input provided. Please set one of --pangenome, --genomes or --proteome input parameters."
    }
